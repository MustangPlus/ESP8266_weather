...
#define weatherUpdateTime 10000 //更新天气预报时间，单位ms
...
void wifiConfigInit();//wifi初始化配置
void wifiRequest();//发送请求消息并获取数据JSON
void readReponseContent();//读取响应消息的字符串数组指针
void monitorDisplay();//液晶显示
void flip();
...
void setup(void) {
  ...
  /****************温湿度获取部分****************/
  dht.begin();
//  flipper.attach(10, flip);//启用定时中断
  lastTime = millis();
}

void loop(void) {
  monitorDisplay();
  if((millis()-lastTime)>weatherUpdateTime){
    monitorDisplay();
    wifiRequest();
    lastTime = millis();
    }
